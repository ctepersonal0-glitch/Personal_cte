...
async function loadUsersFromCloud() {
  try {
    const res = await fetch(`${APPS_SCRIPT_URL}?action=getUsers&token=cte-token-seguro-2026`, { redirect: 'follow' });
...
async function saveUsers() {
  ...
  try {
    await fetch(`${APPS_SCRIPT_URL}?action=saveUsers&token=cte-token-seguro-2026&users=${encodeURIComponent(JSON.stringify(USERS))}`, { redirect: 'follow' });
  } catch(e) {
...
// ==================== SOLICITUDES DE REGISTRO (Firebase) ====================
// Las solicitudes se guardan en Firebase Firestore desde el formulario del HTML.
// Aquí las leemos, aprobamos y rechazamos usando las funciones expuestas por el módulo Firebase.

// Esperar a que Firebase esté listo
function _waitFirebase(fn, maxTries) {
  maxTries = maxTries || 20;
  if (typeof window._obtenerSolicitudes === 'function') { fn(); return; }
  if (maxTries <= 0) { console.warn('Firebase no cargó a tiempo'); fn(); return; }
  setTimeout(function() { _waitFirebase(fn, maxTries - 1); }, 300);
}

async function getSolicitudes() {
  if (typeof window._obtenerSolicitudes === 'function') {
    try {
      return await window._obtenerSolicitudes();
    } catch(e) {
...
async function aprobarSolicitud(idFirebase) {
  // idFirebase es el ID de documento de Firestore (string)
  const solicitudes = await getSolicitudes();
  const solicitud = solicitudes.find(s => s.id === idFirebase);
  if (!solicitud) { showToast('SOLICITUD NO ENCONTRADA', 'error'); return; }

  const nombre = solicitud.nombre || solicitud.area || 'SIN NOMBRE';
  const correo = solicitud.correo || solicitud.email || '';
  const motivo = solicitud.motivo || '';

  // Extraer área, provincia, código del campo motivo si existen
  const areaMatch    = motivo.match(/Área:\s*([^|]+)/i);
  const provMatch    = motivo.match(/Provincia:\s*([^|]+)/i);
  const codMatch     = motivo.match(/Código:\s*([^|]+)/i);
  ...
  const baseUsername   = correo ? correo.split('@')[0] : ('user' + Date.now());
  ...
  // Marcar como aprobada en Firebase
  if (typeof window._aprobarSolicitud === 'function') {
    try { await window._aprobarSolicitud(idFirebase); } catch(e) { console.warn(e); }
  }

  addLog(username, nombreCompleto, 'aprobacion', correo,
    'CUENTA APROBADA POR ADMINISTRADOR' + (area ? ' - ÁREA: ' + area : '') +
    (provincia ? ', PROVINCIA: ' + provincia : '') + (codigo ? ', CÓDIGO: ' + codigo : ''));

  renderAdmin();
  showToast('USUARIO ' + nombreCompleto + ' APROBADO.', 'success');
}

async function rechazarSolicitud(idFirebase) {
  const solicitudes = await getSolicitudes();
  const solicitud = solicitudes.find(s => s.id === idFirebase);
  if (!solicitud) { showToast('SOLICITUD NO ENCONTRADA', 'error'); return; }
  ...
  openModal('RECHAZAR SOLICITUD', '¿RECHAZAR LA SOLICITUD DE ' + nombre.toUpperCase() + '?', async function() {
    if (typeof window._rechazarSolicitud === 'function') {
      try { await window._rechazarSolicitud(idFirebase); } catch(e) { console.warn(e); }
    }
    renderAdmin();
...
async function renderSolicitudes() {
  ...
  try {
    ...
    container.innerHTML = '<div class="file-empty">❌ ERROR AL CARGAR SOLICITUDES</div>';
    return;
  }

  // Actualizar badge del nav-admin
  const pendientes = solicitudes.filter(function(s) { return s.estado === 'pendiente'; });
  const navAdmin = document.getElementById('nav-admin');
  if (navAdmin) {
    navAdmin.textContent = pendientes.length > 0
      ? '⚙ ADMIN (' + pendientes.length + ' ⚠️)'
      : '⚙ ADMIN';
  }
  ...
  const colorEstado = { pendiente: '#e67e22', aprobada: '#27ae60', rechazada: '#c0392b' };

  container.innerHTML = '<div class="files-grid">' + solicitudes.map(function(s) {
    const fecha = s.fecha && s.fecha.toDate
      ? s.fecha.toDate().toLocaleString('es-EC')
      : (s.fecha || '—');
    const estado  = s.estado || 'pendiente';
    const color   = colorEstado[estado] || '#888';
    const nombre  = esc(s.nombre || s.area || '—');
    const correo  = esc(s.correo || s.email || '—');
    const motivo  = esc(s.motivo || '—');
    const btnAprobar  = '<button class="file-card-btn file-card-btn-open btn-approve" onclick="aprobarSolicitud('' + s.id + '')">✓ APROBAR</button>';
    const btnRechazar = '<button class="file-card-btn file-card-btn-del btn-reject" onclick="rechazarSolicitud('' + s.id + '')">✗ RECHAZAR</button>';
    const btns = estado === 'pendiente' ? btnAprobar + btnRechazar : '';

    return '<div class="file-card" style="border-left:4px solid ' + color + '">' +
      '<div class="file-card-top">' +
        '<div class="file-type-icon file-type-other">📧</div>' +
        '<div>' +
          '<div class="file-card-name">' + nombre + '</div>' +
          '<div class="file-card-meta">' +
            'CORREO: ' + correo + '<br>' +
            'MOTIVO: ' + motivo + '<br>' +
            'FECHA: ' + fecha + '<br>' +
            '<span style="padding:.1rem .5rem;border-radius:10px;background:' + color + ';color:#fff;font-size:.72rem">' + estado.toUpperCase() + '</span>' +
          '</div>' +
        '</div>' +
      '</div>' +
      (btns ? '<div class="file-card-actions">' + btns + '</div>' : '') +
    '</div>';
  }).join('') + '</div>';
}
...
function extractDriveId(url){
  ...
  if(m1) return { id: m1[1], isFolder: false };
  if(m2) return { id: m2[1], isFolder: false };
  if(m3) return { id: m3[1], isFolder: true };
  return null;
}
function driveViewUrl(url, tipo){
  const result = extractDriveId(url);
  if(!result) return url;
  const { id, isFolder } = result;
  // Si es carpeta, abrir directamente en Drive
  if(isFolder) return 'https://drive.google.com/drive/folders/' + id + '?usp=sharing';
  if(tipo === 'pdf' || tipo === 'img') return 'https://drive.google.com/file/d/' + id + '/view?usp=sharing';
  // Excel: abrir con Google Sheets; Word: abrir con Google Docs
  if(tipo === 'excel') return 'https://docs.google.com/spreadsheets/d/' + id + '/edit?usp=sharing';
  if(tipo === 'word') return 'https://docs.google.com/document/d/' + id + '/edit?usp=sharing';
...
function verifyDriveLink(url){
  const result = extractDriveId(url);
  if(result) showToast(`ID DE DRIVE DETECTADO: ${result.id}${result.isFolder ? ' (CARPETA)' : ''} — ACCESIBLE`, 'success');
