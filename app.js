...
// Variables globales de Firebase (se inicializan cuando el SDK cargue)
let _fbApp = null, _fbDatabase = null;
let _fbReady = false;
let _fbPendingLogs = [];      // logs en cola hasta que Firebase esté listo
...
async function saveUsers() {
  ...
  // 🔥 Firebase: guardar usuarios en tiempo real
  if (_fbReady && _fbDatabase) {
    try {
      const { ref, set } = await import("https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js");
      await set(ref(_fbDatabase, 'usuarios'), USERS);
    } catch(e) { console.warn('Firebase saveUsers error:', e); }
  }
}
...
async function getSolicitudes() {
  // 🔥 Firebase: leer solicitudes desde Realtime DB
  if (_fbReady && _fbDatabase) {
    try {
      const { ref, get } = await import("https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js");
      const snapshot = await get(ref(_fbDatabase, 'solicitudes'));
      if (snapshot.exists()) {
        const val = snapshot.val();
        const arr = Array.isArray(val) ? val : Object.values(val);
        _solicitudesCache = arr;
        return arr;
      }
      return [];
    } catch(e) { console.warn('Firebase getSolicitudes error:', e); }
  }
...
async function saveSolicitudes(solicitudes) {
  ...
  // 🔥 Firebase: guardar solicitudes en tiempo real (PRIORIDAD)
  if (_fbReady && _fbDatabase) {
    try {
      const { ref, set } = await import("https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js");
      await set(ref(_fbDatabase, 'solicitudes'), solicitudes);
    } catch(e) { console.warn('Firebase saveSolicitudes error:', e); }
  }

  // Respaldo: Apps Script (Google Sheets)
  try {
...
function addLog(usuario, nombre, estado, email='', details=''){
  ...
  // 1. Guardar en localStorage (instantáneo, siempre)
  logs.push(entry);
  saveLogs(logs);

  // 2. 🔥 Firebase: push a Realtime DB (tiempo real)
  if (_fbReady && _fbDatabase) {
    import("https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js")
      .then(({ ref, push }) => push(ref(_fbDatabase, 'logs'), entry))
      .catch(e => console.warn('Firebase addLog error:', e));
  } else {
    // Encolar para enviar cuando Firebase esté listo
    _fbPendingLogs.push(entry);
  }
}

// Enviar logs en cola cuando Firebase esté disponible
async function _flushPendingLogs() {
  if (!_fbPendingLogs.length || !_fbDatabase) return;
  try {
    const { ref, push } = await import("https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js");
    for (const entry of _fbPendingLogs) {
      await push(ref(_fbDatabase, 'logs'), entry);
    }
    _fbPendingLogs = [];
  } catch(e) { console.warn('Firebase flush error:', e); }
}

// ==================== LISTENERS TIEMPO REAL (FIREBASE) ====================
async function initFirebaseListeners() {
  if (_fbListenersActive || !_fbDatabase) return;
  _fbListenersActive = true;

  try {
    const { ref, onValue } = await import("https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js");

    // 🔔 Listener: nuevas solicitudes → notificar al admin al instante
    onValue(ref(_fbDatabase, 'solicitudes'), (snapshot) => {
      const val = snapshot.val();
      const solicitudes = val ? (Array.isArray(val) ? val : Object.values(val)) : [];
      _solicitudesCache = solicitudes;

      if (currentUser && currentUser.rol === 'admin') {
        // Actualizar lista de solicitudes si el admin está en pantalla
        renderSolicitudes();

        // Badge visual en el menú de admin
        const navAdmin = document.getElementById('nav-admin');
        if (navAdmin) {
          if (solicitudes.length > 0) {
            navAdmin.innerHTML = `⚙ ADMIN <span style="background:#c0392b;color:white;border-radius:50%;padding:0 5px;font-size:.75rem;margin-left:4px">${solicitudes.length}</span>`;
          } else {
            navAdmin.innerHTML = '⚙ ADMIN';
          }
        }

        // Toast solo si hay solicitudes nuevas y el admin NO está en pestaña admin
        const adminTabActive = document.getElementById('tab-admin')?.classList.contains('active');
        if (solicitudes.length > 0 && !adminTabActive) {
          showToast(`🔔 ${solicitudes.length} SOLICITUD(ES) DE REGISTRO PENDIENTE(S)`, 'info', 5000);
        }
      }
    });

    // 📋 Listener: logs en tiempo real → actualizar bitácora del admin
    onValue(ref(_fbDatabase, 'logs'), (snapshot) => {
      const val = snapshot.val();
      if (!val) return;
      const logs = Object.values(val).sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
      saveLogs(logs);
    ...
    console.log('🔥 Firebase listeners activos — tiempo real habilitado');
  } catch(e) {
    console.warn('Error iniciando Firebase listeners:', e);
  }
}
...
function entrarAlSistema(found, foto){
  ...
  if (isAdmin) {
    renderAdmin();
    // 🔥 Activar listeners de tiempo real al entrar como admin
...
async function initFirebase() {
  try {
    ...
    _fbDatabase = getDatabase(_fbApp);
    _fbReady = true;

    // Enviar logs que estaban en cola
    await _flushPendingLogs();

    console.log('🔥 Firebase conectado correctamente');
    showToast('🔥 Tiempo real activado', 'success', 2500);
  } catch(e) {
    console.warn('Firebase no disponible — funcionando sin tiempo real:', e);
  }
}
...
(async function init(){
  ...
  // 🔥 Iniciar Firebase en paralelo (no bloquea la UI)
  initFirebase();
})();
