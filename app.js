...
async function getSolicitudes() {
  try {
    const res = await fetch(`${APPS_SCRIPT_URL}?action=getSolicitudes&token=cte-token-seguro-2026`, { redirect: 'follow' });
    const data = await res.json();
...
async function saveSolicitudes(solicitudes) {
  ...
  try {
    const url = `${APPS_SCRIPT_URL}?action=saveSolicitudes&token=cte-token-seguro-2026&solicitudes=${encodeURIComponent(JSON.stringify(solicitudes))}`;
    const saveRes = await fetch(url, { redirect: 'follow' });
    const saveData = await saveRes.json();
    if (!saveData.ok) {
      console.error('Apps Script error en saveSolicitudes:', saveData);
      showToast('⚠️ ERROR AL GUARDAR EN SERVIDOR: ' + (saveData.error || 'desconocido'), 'error', 5000);
    }
  } catch(e) {
